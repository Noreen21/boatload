...
#      namespaced: false
  ...
  - name: boatload
    ...
    - objectTemplate: workload-deployment-selector.yml
      ...
      inputVars:
        serviceAccountName: boatload-1
        automountServiceAccountToken: false
        runtimeClassName: performance-perf-profile
        pod_replicas: {{ pod_replicas }}
        containers: 1
        image: e27-h11-r750.alias.bos.scalelab.redhat.com:5000/perf-tools/stress-ng:latest
  ...
  - name: boatload-bu-pvc-vfs-exec
    jobType: create
    jobIterations: 5
    namespace: boatload-bu-pvc-vfs-exec
    namespacedIterations: true
    cleanup: false
    podWait: false
    waitWhenFinished: true
    ...
    errorOnVerify: false
    objects:
    - objectTemplate: workload-deployment-selector-bu-pvc-vfs-exec.yml
      replicas: {{ deployments }}
      inputVars:
        pod_replicas: 1
        containers: 2
        ...
        enable_liveness_probe: {{ liveness_probe_args | length > 0 }}
        enable_readiness_probe: {{ readiness_probe_args | length > 0 }}
        startup_probe_args: ['initialDelaySeconds: 0', 'periodSeconds: 10', 'timeoutSeconds: 2', 'failureThreshold: 12', 'successThreshold: 1', 'exec:', '  command:', '  - test', '  - -f', '  - /tmp/startup']
        liveness_probe_args: ['initialDelaySeconds: 0', 'periodSeconds: 10', 'timeoutSeconds: 2', 'failureThreshold: 12', 'successThreshold: 1', 'exec:', '  command:', '  - test', '  - -f', '  - /tmp/liveness']
        readiness_probe_args: ['initialDelaySeconds: 0', 'periodSeconds: 10', 'timeoutSeconds: 2', 'failureThreshold: 12', 'successThreshold: 1', 'exec:', '  command:', '  - test', '  - -f', '  - /tmp/readiness']
        startup_probe_port: {{ startup_probe_port_enable }}
        liveness_probe_port: {{ liveness_probe_port_enable }}
        readiness_probe_port: {{ readiness_probe_port_enable }}
        default_selector: "{{ default_selector }}"
  ...
  - name: boatload-bu-pvc-vfs
    jobType: create
    jobIterations: 5
    namespace: boatload-bu-pvc-vfs
    namespacedIterations: true
    cleanup: false
    podWait: false
    waitWhenFinished: true
    ...
    errorOnVerify: false
    objects:
    - objectTemplate: workload-deployment-selector-bu-pvc-vfs.yml
      replicas: {{ deployments }}
      inputVars:
        pod_replicas: 1
        containers: 2
        ...
        set_requests_hps: {{ hugepages_requests > 0 }}
        set_limits_hps: {{ hugepages_limits > 0 }}
        net_resource: net_vfs_resource
        net_name: network-vfs
        resources:
          requests:
            cpu: {{ cpu_requests }}m
            memory: {{ memory_requests }}Mi
  ...
  - name: boatload-bu-pvc
    jobType: create
    jobIterations: 10
    namespace: boatload-bu-pvc
    namespacedIterations: true
    cleanup: false
    podWait: false
    waitWhenFinished: true
    ...
    errorOnVerify: false
    objects:
    - objectTemplate: workload-deployment-selector-bu-pvc.yml
      replicas: {{ deployments }}
      inputVars:
        pod_replicas: 1
        containers: 2
...
- name: cleanup-boatload-bu-pvc-vfs-exec
  jobType: delete
  waitForDeletion: true
  qps: 20
  burst: 20
  objects:
  - kind: Namespace
    labelSelector: {kube-burner-job: boatload-bu-pvc-vfs-exec}
    apiVersion: v1

- name: cleanup-boatload-bu-pvc-vfs
  jobType: delete
  waitForDeletion: true
  qps: 20
  burst: 20
  objects:
  - kind: Namespace
    labelSelector: {kube-burner-job: boatload-bu-pvc-vfs}
    apiVersion: v1

- name: cleanup-boatload-bu-pvc
  jobType: delete
  waitForDeletion: true
  qps: 20
  burst: 20
  objects:
  - kind: Namespace
    labelSelector: {kube-burner-job: boatload-bu-pvc}
    apiVersion: v1
      ...
      - name: boatload-{{ $element }}
        ...
        resources:
          ...
          limits:
            ...
            {{ end }}
        securityContext:
          privileged: {{ $data.securityContext.privileged }}
        env:
        - name: tool
          value: "stress-ng"
        - name: DURATION
...
workload_deployment_bu_pvc_vfs = """---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: boatload-{{ .Iteration }}-{{ .Replica }}-{{ .JobName }}
...
workload_deployment_bu_pvc_vfs_exec = """---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: boatload-{{ .Iteration }}-{{ .Replica }}-{{ .JobName }}
...
def write_csv_results(result_file_name, results):
  header = ["start_ts", "workload_complete_ts", "measurement_complete_ts", "cleanup_complete_ts", "end_ts",
      ...
      "secrets", "pvcs", "image", "cpu_requests", "memory_requests", "cpu_limits", "memory_limits", "startup_probe",
...
#  ]
  ...
  default_metrics_collected = [
      "nodeReadyStatus", "nodeCoresUsed", "nodeMemoryConsumed", "nodeCPU", "kubeletCoresUsed", "kubeletMemory", "crioCoresUsed", "crioMemory", "KubeapiserverPodCPUcores", "KubeapiserverPodMemory", "ControlPlaneCPU", "ControlPlaneMemory", "cpu-masters", "memory-masters", "cpu-kube-controller-manager", "memory-kube-controller-manager", "cpu-openshift-controller-manager", "memory-openshift-controller-manager", "cpu-kube-apiserver", "memory-kube-apiserver", "cpu-openshift-apiserver", "memory-openshift-apiserver", "cpu-etcd", "memory-etcd", "cpu-ovn-control-plane", "memory-ovn-control-plane", "cpu-ovnkube-node", "memory-ovnkube-node", "cpu-ovn-kubernetes", "memory-ovn-kubernetes", "cpu-logging", "memory-logging", "cpu-collector-logging", "cpu-monitoring", "memory-monitoring", "cpu-prometheus", "cpu-sriov", "memory-sriov", "cpu-ptp", "memory-ptp", "cpu-dns", "memory-dns", "cpu-local-storage", "memory-local-storage", "cpu-etcd-operator", "memory-etcd-operator", "cpu-dns-operator", "memory-dns-operator", "cpu-oauth-apiserver", "memory-oauth-apiserver", "cpu-multus", "memory-multus", "cpu-authentication", "memory-authentication", "cpu-ingress", "memory-ingress", "cpu-kube-scheduler", "memory-kube-scheduler", "cpu-nto", "memory-nto", "cpu-mco", "memory-mco", "cpu-vran", "memory-vran", "cpu-ksv", "memory-ksv", "cpu-ksv-operator", "memory-ksv-operator", "cpu-service-ca", "memory-service-ca", "cpu-service-ca-operator", "memory-service-ca-operator", "cpu-marketplace", "memory-marketplace", "cpu-kube-scheduler-operator", "memory-kube-scheduler-operator", "cpu-ingress-canary", "memory-ingress-canary", "cpu-ingress-operator", "memory-ingress-operator", "cpu-authentication-operator", "memory-authentication-operator", "cpu-kube-apiserver-operator", "memory-kube-apiserver-operator", "cpu-openshift-apiserver-operator", "memory-openshift-apiserver-operator", "cpu-openshift-controller-manager-operator", "memory-openshift-controller-manager-operator", "cpu-kube-controller-manager-operator", "memory-kube-controller-manager-operator", "cpu-nni", "memory-nni", "cpu-ccmo", "memory-ccmo", "cpu-cco", "memory-cco", "cpu-cma", "memory-cma", "cpu-ocv", "memory-ocv", "cpu-no", "memory-no", "cpu-rcm", "memory-rcm", "cpu-olm", "memory-olm", "cpu-co", "memory-co", "cpu-clo", "cpu-disk-manager", "cpu-lso", "cpu-prom-ad", "cpu-prom-op", "cpu-thanos", "cpu-osm", "cpu-ksm", "cpu-node-exporter", "cpu-cmo", "cpu-prom-aw", "cpu-olm-co", "cpu-olm-op", "cpu-olm-psm", "cpu-olm-ps", "cpu-ptp-linux", "cpu-ptp-op", "cpu-sriov-nri", "cpu-sriov-ow", "cpu-sriov-dp", "cpu-sriov-ncd", "cpu-sriov-op", "cpu-marketplace-op", "cpu-marketplace-index", "cpu-mco-mcc", "cpu-mco-mcd", "cpu-mco-pod", "cpu-mco-mcs", "cpu-multus-pod", "cpu-multus-ac", "cpu-multus-acp", "cpu-multus-nmd"
# "cpu-ovn-nbdb", "cpu-ovn-sbdb", "cpu-ovn-northd", "cpu-ovnkube-master", "cpu-ovnkube-cluster-manager", "cpu-ovnkube-node-container", "cpu-ovn-controller", "memory-ovn-nbdb", "memory-ovn-sbdb", "memory-ovn-northd", "memory-ovnkube-master", "memory-ovnkube-cluster-manager", "memory-ovnkube-node-container", "memory-ovn-controller"
  ]
...
#                      default="quay.io/redhat-performance/test-gohttp-probe:v0.0.2", help="The container image to use")
  parser.add_argument("-i", "--container-image", type=str,
                      default="e27-h11-r750.alias.bos.scalelab.redhat.com:5000/redhat-performance/test-gohttp-probe:latest", help="The container image to use")
  ...
  parser.add_argument("--csv-results-file-1", type=str, default="results.csv", help="Determines results csv to append to")
  parser.add_argument("--csv-results-file-2", type=str, default="results.csv", help="Determines results csv to append to")
  parser.add_argument("--csv-results-file-3", type=str, default="results.csv", help="Determines results csv to append to")
...
#    logger.info("Created {}/workload-pv.yml".format(tmp_directory))
    ...
    with open("{}/workload-deployment-selector-bu-pvc-vfs-exec.yml".format(tmp_directory), "w") as file1:
      file1.writelines(workload_deployment_bu_pvc_vfs_exec)
    logger.info("Created {}/workload-deployment-selector-bu-pvc-vfs-exec.yml".format(tmp_directory))
    with open("{}/workload-deployment-selector-bu-pvc-vfs.yml".format(tmp_directory), "w") as file1:
      file1.writelines(workload_deployment_bu_pvc_vfs)
    logger.info("Created {}/workload-deployment-selector-bu-pvc-vfs.yml".format(tmp_directory))
    with open("{}/workload-deployment-selector-bu-pvc.yml".format(tmp_directory), "w") as file1:
      file1.writelines(workload_deployment_bu_pvc)
    logger.info("Created {}/workload-deployment-selector-bu-pvc.yml".format(tmp_directory))
    ...
    workload_bu_measurements_json = "{}/metrics/podLatencyQuantilesMeasurement-boatload-bu.json".format(tmp_directory)
    workload_bu_pvc_vfs_exec_measurements_json = "{}/metrics/podLatencyQuantilesMeasurement-boatload-bu-pvc-vfs-exec.json".format(tmp_directory)
    workload_bu_pvc_vfs_measurements_json = "{}/metrics/podLatencyQuantilesMeasurement-boatload-bu-pvc-vfs.json".format(tmp_directory)
    workload_bu_pvc_measurements_json = "{}/metrics/podLatencyQuantilesMeasurement-boatload-bu-pvc.json".format(tmp_directory)
...
#   end_time = measurement_end_time
  ...
  for measurement in kb_measurements:
    pod_latencies_bu[measurement] = {}
    for stat in kb_stats:
      pod_latencies_bu[measurement][stat] = 0
  if not cliargs.no_workload_phase and not cliargs.dry_run:
    logger.info("Reading {} for measurement data".format(workload_bu_measurements_json))
    with open(workload_bu_measurements_json) as measurements_file:
      measurements = json.load(measurements_file)
    for measurement in measurements:
      if measurement["jobName"] == "boatload-bu":
       pod_latencies_bu[measurement["quantileName"]] = {}
       for stat in kb_stats:
         pod_latencies_bu[measurement["quantileName"]][stat] = measurement[stat]
  logger.debug("kube-burner podLatency measurements: {}".format(pod_latencies_bu))

  kb_measurements = ["PodScheduled", "Initialized", "ContainersReady", "Ready"]
  kb_stats = ["avg", "max", "P50", "P95", "P99"]
  pod_latencies_bu_pvc_vfs_exec = {}
  for measurement in kb_measurements:
    pod_latencies_bu_pvc_vfs_exec[measurement] = {}
    for stat in kb_stats:
      pod_latencies_bu_pvc_vfs_exec[measurement][stat] = 0
  if not cliargs.no_workload_phase and not cliargs.dry_run:
    logger.info("Reading {} for measurement data".format(workload_bu_pvc_vfs_exec_measurements_json))
    with open(workload_bu_pvc_vfs_exec_measurements_json) as measurements_file:
      measurements = json.load(measurements_file)
    for measurement in measurements:
      if measurement["jobName"] == "boatload-bu-pvc-vfs-exec":
       pod_latencies_bu_pvc_vfs_exec[measurement["quantileName"]] = {}
       for stat in kb_stats:
         pod_latencies_bu_pvc_vfs_exec[measurement["quantileName"]][stat] = measurement[stat]
  logger.debug("kube-burner podLatency measurements: {}".format(pod_latencies_bu_pvc_vfs_exec))

  kb_measurements = ["PodScheduled", "Initialized", "ContainersReady", "Ready"]
  kb_stats = ["avg", "max", "P50", "P95", "P99"]
  pod_latencies_bu_pvc_vfs = {}
  for measurement in kb_measurements:
    pod_latencies_bu_pvc_vfs[measurement] = {}
    for stat in kb_stats:
      pod_latencies_bu_pvc_vfs[measurement][stat] = 0
  if not cliargs.no_workload_phase and not cliargs.dry_run:
    logger.info("Reading {} for measurement data".format(workload_bu_pvc_vfs_measurements_json))
    with open(workload_bu_pvc_vfs_measurements_json) as measurements_file:
      measurements = json.load(measurements_file)
    for measurement in measurements:
      if measurement["jobName"] == "boatload-bu-pvc-vfs":
       pod_latencies_bu_pvc_vfs[measurement["quantileName"]] = {}
       for stat in kb_stats:
         pod_latencies_bu_pvc_vfs[measurement["quantileName"]][stat] = measurement[stat]
  logger.debug("kube-burner podLatency measurements: {}".format(pod_latencies_bu_pvc_vfs))

  kb_measurements = ["PodScheduled", "Initialized", "ContainersReady", "Ready"]
  kb_stats = ["avg", "max", "P50", "P95", "P99"]
  pod_latencies_bu_pvc = {}
  for measurement in kb_measurements:
    pod_latencies_bu_pvc[measurement] = {}
    for stat in kb_stats:
      pod_latencies_bu_pvc[measurement][stat] = 0
  if not cliargs.no_workload_phase and not cliargs.dry_run:
    logger.info("Reading {} for measurement data".format(workload_bu_pvc_measurements_json))
    with open(workload_bu_pvc_measurements_json) as measurements_file:
      measurements = json.load(measurements_file)
    for measurement in measurements:
      if measurement["jobName"] == "boatload-bu-pvc":
       pod_latencies_bu_pvc[measurement["quantileName"]] = {}
       for stat in kb_stats:
         pod_latencies_bu_pvc[measurement["quantileName"]][stat] = measurement[stat]
  logger.debug("kube-burner podLatency measurements: {}".format(pod_latencies_bu_pvc))
...
#      metric_json = os.path.join(metrics_dir, "{}-{}.json".format(metric, workload_UUID))
  ...
  if not cliargs.cleanup:
    ...
    results_1 = [int(start_time * 1000), int(workload_end_time * 1000), int(measurement_end_time * 1000),
        int(cleanup_end_time * 1000), int(end_time * 1000), datetime.utcfromtimestamp(start_time),
        datetime.utcfromtimestamp(workload_end_time), datetime.utcfromtimestamp(measurement_end_time),
        datetime.utcfromtimestamp(cleanup_end_time), datetime.utcfromtimestamp(end_time), cliargs.csv_title,
        workload_UUID, workload_duration, measurement_duration, cleanup_duration, metrics_duration, total_time,
        ...
        cliargs.link_flap_firewall, cliargs.link_flap_network, indexing_enabled, cliargs.dry_run, link_flap_count,
        nodenotready_node_controller_count, nodenotready_kubelet_count, nodeready_count, marked_evictions, killed_pod, started_pod, notready_pod]
    results_2 = [int(start_time * 1000), int(workload_end_time * 1000), int(measurement_end_time * 1000),
        int(cleanup_end_time * 1000), int(end_time * 1000), datetime.utcfromtimestamp(start_time),
        datetime.utcfromtimestamp(workload_end_time), datetime.utcfromtimestamp(measurement_end_time),
        datetime.utcfromtimestamp(cleanup_end_time), datetime.utcfromtimestamp(end_time), cliargs.csv_title,
        workload_UUID, workload_duration, measurement_duration, cleanup_duration, metrics_duration, total_time,
        ...
        cliargs.link_flap_firewall, cliargs.link_flap_network, indexing_enabled, cliargs.dry_run, link_flap_count,
        nodenotready_node_controller_count, nodenotready_kubelet_count, nodeready_count, marked_evictions, killed_pod, started_pod, notready_pod]
    results_3 = [int(start_time * 1000), int(workload_end_time * 1000), int(measurement_end_time * 1000),
        int(cleanup_end_time * 1000), int(end_time * 1000), datetime.utcfromtimestamp(start_time),
        datetime.utcfromtimestamp(workload_end_time), datetime.utcfromtimestamp(measurement_end_time),
        datetime.utcfromtimestamp(cleanup_end_time), datetime.utcfromtimestamp(end_time), cliargs.csv_title,
        workload_UUID, workload_duration, measurement_duration, cleanup_duration, metrics_duration, total_time,
    ...
    for measurement in kb_measurements:
      for stat in kb_stats:
        results.append(pod_latencies_bu[measurement][stat])
    write_csv_results(cliargs.csv_results_file, results)
    for measurement in kb_measurements:
      for stat in kb_stats:
        results_1.append(pod_latencies_bu_pvc_vfs_exec[measurement][stat])
    write_csv_results(cliargs.csv_results_file_1, results_1)
    for measurement in kb_measurements:
      for stat in kb_stats:
        results_2.append(pod_latencies_bu_pvc_vfs[measurement][stat])
    write_csv_results(cliargs.csv_results_file_2, results_2)
    for measurement in kb_measurements:
      for stat in kb_stats:
        results_3.append(pod_latencies_bu_pvc[measurement][stat])
    write_csv_results(cliargs.csv_results_file_3, results_3)
