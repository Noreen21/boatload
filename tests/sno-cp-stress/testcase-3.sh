...
for iteration in `seq 1 ${iterations}`; do
  ...
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_pods} namespaces, 1 deploy, 1 pod, 2 container, gohttp image, http probes, pvcs, 2 configmaps, 4 secrets, mixed resources"
   ...
   oc label ns boatload-1 security.openshift.io/scc.podSecurityLabelSync=false pod-security.kubernetes.io/enforce=privileged pod-security.kubernetes.io/audit=privileged pod-security.kubernetes.io/warn=privileged --overwrite
   oc create sa boatload-1 -n boatload-1
   oc adm policy add-scc-to-user privileged system:serviceaccount:boatload-1:boatload-1
   sleep 180
   ../../boatload/boatload-stress-ng-sno-du-profile.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-gubu-pv-probes-2cm-4s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 2 -v 1 -l -r ${resources} ${probes} ${configmaps_secrets} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations} --metrics-profile metrics_controlplane.yaml
...
for iteration in `seq 1 ${iterations}`; do
  ...
  probes=" --startup-probe http,0,10,2,12,1 --liveness-probe http,0,10,2,3,1 --readiness-probe http,0,10,2,3,1 "
  resources=" --cpu-requests 100 --memory-requests 100 "
  echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - long test - ${total_pods} namespaces, 1 deploy, 1 pod, 2 container, gohttp image, 1 service, 1 route, http probes, pvcs, 2 configmaps, 4 secrets, mixed resources"
  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
   oc delete -f SriovNodeNetworks_mixed.yaml
   sleep 5
   oc create -f SriovNodeNetworks_mixed.yaml
