...
probe_type="http"
probes=()
probes+=("--startup-probe ${probe_type},0,10,2,12 --liveness-probe off --readiness-probe off")
...
probes+=("--startup-probe ${probe_type},0,10,2,12 --liveness-probe ${probe_type},0,10,2,3 --readiness-probe off")
...
probes_csv_titles=()
probes_csv_titles+=("${probe_type}-s10-l0-r0-2")
#probes_csv_titles+=("${probe_type}-s5-l0-r0")
#probes_csv_titles+=("${probe_type}-s1-l0-r0")
probes_csv_titles+=("${probe_type}-s10-l10-r0-2")
...
for (( index=0; index<${#probes[@]}; index++)); do
  for iteration in `seq 1 ${iterations}`; do
    ...
    sleep 500
    ../../boatload/boatload-sno-du-profile.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-gubu-${probes_csv_titles[$index]}-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -v 1 -l -r ${probes[$index]} ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations}
